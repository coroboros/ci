...
- One reusable entry-point workflow (`javascript-npm-packages.yml`) with three jobs gated by trigger event:
  - **`preflight`** (on branches) — `setup-base` + `pnpm run lint` + `pnpm run build` (if `scripts.build` exists) + `pnpm test`.
...
- One composite action (`setup-base`) holds the shared preamble: `.node-version` resolution + `actions/setup-node` + corepack + pnpm store cache + `.npmrc` generation (from `NPM_CONFIG_FILE` secret + `vars.NPM_EXTRA_CONFIG`) + README presence check + `pnpm install --frozen-lockfile --ignore-scripts`. Called by `preflight` and `publish`.
...
- Top-level `env:` block declares `CI_TEXT_RED` / `CI_TEXT_GREEN` / `CI_TEXT_CLEAR` once; inherited by every job and step.
...
### Documentation
- `README.md` describes the 3-job model + Quick start + `setup-base` composable.
- `docs/examples.md` — consumer wire-up for `javascript-npm-packages.yml` + standalone `security.yml` + composing with `setup-base`.
