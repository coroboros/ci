...
- `security-gate` — a blocking pre-release gate in its own reusable workflow (`security-gate.yml`) that each `publish` `needs:` as one unit: `supply-chain` (auto-routed — `cargo-deny` advisories+bans+sources for a Rust repo, `osv-scanner` otherwise) + `secret-scan` (gitleaks, full history). A supply-chain finding or a leaked secret blocks the release through the job graph, not the consumer's branch protection — a dev can't bypass it. Parity with the GitLab `security-gate` stage. The advisory `security.yml` reports in parallel and never blocks.
...
- `javascript-npm-packages` — gate `publish` on the `security-gate` (osv-scanner + gitleaks, `needs:`). A known vulnerability or leaked secret now blocks the npm release; previously osv-scanner ran only in the parallel `security` job and could not stop a publish.
...
### Refactor
- `security` — split the blocking scans into a `security-gate.yml` reusable workflow and keep `security.yml` as the advisory layer, so the gate scans run once instead of duplicating across an inline gate and the called `security.yml`. Each `publish` `needs:` the single `security-gate` job; the supply-chain scan auto-routes by ecosystem (`cargo-deny` for Rust, `osv-scanner` otherwise). `licenses` moves off the gate to advisory (`continue-on-error`) — a license is compliance, not an attack vector, so it reports without blocking, parity with GitLab keeping license scanning off the blocking gate.
- `security` — extract the gitleaks, osv-scanner, and cargo-deny scanners into `security/*` composites (single source), reused by `security-gate.yml`, `security.yml`, and self-CI so each scanner is defined once. The osv composite scans only when a supported manifest is present (`pnpm-lock.yaml`, `Cargo.lock`, `go.mod`, and the rest), so a manifest-less repo skips instead of failing on osv's no-manifest error. The cargo-deny composite (`security/rust/cargo-deny`) imposes a canonical `security/deny.toml` via `--config` — a consumer `deny.toml` is ignored and a `deny.exceptions.toml` is rejected — and takes a `checks` input so the gate runs `advisories bans sources` and the advisory layer runs `licenses`. `self-security.yml` runs the composites via local refs to self-test them pre-release.
...
- `security/rust/cargo-deny` — drop the redundant `--deny unmaintained --deny unsound` CLI flags. The imposed `deny.toml` already errors on both at `"all"` scope (the v2 advisories schema cannot downgrade them), so the policy stays single-sourced in the ruleset.
...
- `self-actions` — new self-CI workflow exercising the composites on every PR via local refs: `release/verify-tag` (HEAD match + moved-HEAD failure), `release/generate-changelog` (SemVer-gate rejection of a non-tag ref), `release/commit-artifacts` (push / no-op / `FILES` word-split against a local bare remote, `contents: read` backstop), `security/rust/cargo-deny` (consumer-override reject guard), and `rust/install-dist` (download + checksum + run on Linux, macOS, Windows — the zip and tarball extraction paths differ). A regression in the reachable logic now fails the PR. Tag-driven paths that read the runner's `GITHUB_REF_NAME`/`GITHUB_SHA` (generate-changelog auto-gen, pin-version stamp) can't be faked on a non-tag event — those reserved defaults aren't overridable into a composite — so they stay validated at real release time.
